...
<div id="huge_it_portfolio" style="display:none;" class="post-content">
	...
	if ( count( $shortcodeportfolios ) ) {
		echo "<select id='huge_it_portfolio-select' class='select_portfolio_list'   name='hugeit_portfolio_id' change-view-nonce='" . $shortecode_change_view_nonce . "'>";
		foreach ( $shortcodeportfolios as $shortcodeportfolio ) {
			echo "<option value='" . $shortcodeportfolio->id . "'>" . $shortcodeportfolio->name . "</option>";
		}
		echo "</select>";
		echo "<button class='insert_shortcode_button' id='hugeitportfolioinsert'>".__('Insert portfolio gallery','portfolio-gallery')."</button>";
	} else {
