import imageMap from '../lib/image_map.json';

export const team = [
    {
        id: "15460",
        name: "Zippy Kimundu",
        role: "Producer / Director",
        image: "kimundu.jpg",
        bio: [
            "Zippy Kimundu is an award-winning Kenyan Filmmaker who has been working in the global industry for nearly two decades. She holds a Masters in Fine Arts from New York University and has worked globally in over 20 countries. She co-directed a short Documentary, ‘a fork, a spoon & a KNIGHT’ with renowned Director Mira Nair, was the Assistant Editor on the Disney Film ‘Queen of Katwe’ and her short film 'Mercy' leads the 'Think Twice Movement'. Her award winning Feature Documentary ‘Our Land, Our Freedom’ premiered at IDFA/Sheffield in 2023 and her 2025 release ‘Widow Champion’ premiered at Hotdocs/Tribeca and continues to create global impact.",
            "Zippy is a Sundance, Chicken & Egg, Logan Elevate/IDA, IDFA, Hotdocs, World Cinema Fund/ACP and FORD FOUNDATION fellow, who has worked with numerous organisations including Universal studios, Nelson Mandela Foundation, Mastercard Foundation, UN, World Bank, US State Department, British council, BBC, Google, British Red Cross, among others. She is the Community Lead at https://www.terracekilifi.com, a Film Educator at https://www.illtellyoumystory.com and Founder at  www.afrofilmsinternational.com, a creative collective working to ignite socio-political consciousness and action, across continents."
        ]
    },
    {
        id: "15459",
        name: "Wanjiru Kimundu",
        role: "Producer",
        image: "wanjiru.jpg",
        bio: [
            "Wanjiru is the in-house Producer and Fixer at Afrofilms International. She has over 10 years experience in the production field working in different capacities including: The AD Department, Production Coordinator, Production Manager and a Live Events Coordinator for the BBC.",
            "Wanjiru has diverse experience in Production Management as she worked at the BBC Media Action for 5 years in different capacities, the last being the Production Co-Ordinator for all their TV and Radio Programming. She’s also been a fixer for various international Crews filming in Kenya - including ‘Facebook’, UNESCO and ‘Globe trotters’.",
            "In 2019, she produced a TVC ‘Educare’ for Standard Chartered Bank, a 6-part short Non-fiction Film Series for the British Council and was the production coordinator for both a 6-part series for SCB’s Business Banking Campaign and a 4-part series of SCB’s Progressive Africa Campaign."
        ]
    },

    {
        id: "15468",
        name: "Steve Ruiyi",
        role: "Director of Photography",
        image: "steve2.jpg.jpeg",
        bio: [
            "Steve has worked for over 10 years as Director of Photography/Cameraman on T.V shows, Documentaries and Features Films. He has shot numerous Documentaries in Kenya, South Africa, Mozambique, Senegal, Zambia, Brazil, Rwanda, Uganda and Swaziland including: UN Free and Equal - 2018- 2019, Permanent Persistent Poison - World Wildlife fund 2018, Wash - BBC media action 2017, The last warriors-produced by Made in Copenhagen – 2016, Train to win by sport brand Australia - 2012, Women of Blue economy- International Committee for the Development Peoples.",
            "Steve has also shot Documentaries for: AWID, USAID, Save the Children, British Council, Safaricom and Standard Chartered Bank. He has been the DOP on numerous T.V shows including, Popo T.V drama, commissioned by Zuku 2015, Parasites T.V drama-Mnet, Dramas of life-NTV, Waridi-Mnet, Seeds of gold-NTV, and Africa Rising-Mnet.",
            "Steve was the DOP on the interactive action Film ‘Simiyu Samurai’ which won 5 Kalasha Awards in 2013 and 2014 - and for Jongo love - the first show shot entirely using a phone and aired on the local T.V channel [NTV]. The show was developed and produced by ‘Well Told Stories’ and was well received world wide. It was featured in BBC international, The Guardian and other international media platforms.",
            "He’s currently making his first feature film ‘Muhia & the Mountains: https://drive.google.com/file/d/18aAhGFw-VcVd6FGQ2J7EYhIVg8RVB8dl/view?ts=60a23236"
        ]
    },
    {
        id: "15469",
        name: "Anthony Thuku",
        role: "Camera Operator / DIT",
        image: imageMap["15469"],
        bio: [
            "Anthony Thuku is a responsible Camera Operator and Data Imaging Technician passionate about delivering outstanding quality and service. With 8 years of industry experience, he has a history of recognition for high performance and is a productive individual with a proven track record of successful project management.",
            "He has achieved success on several major projects including the 'Girls Rule' documentary for the Olympic Channel, 'New Wave' for National Geographic, a United Nations documentary, L'Oréal TVC, CBM global teaching tool, 'Agnes Tirop' for Bloomberg TV, 'My Land, My Freedom', and a Google Plus-Code TVC among many others.",
            "His skills include meeting high expectations with an eye for accuracy, high motivation, confidence, and strict compliance. He is proficient in English (C2) and speaks Swahili as his first language.",
            "Outside of film, Anthony finds inspiration in photography, travel, camping, and hiking.",
            "Contact: thukuantony20@gmail.com | 0710 648 010 | Nairobi, Kenya"
        ]
    },
    {
        id: "15470",
        name: "John Baptist Gichengo",
        role: "Sound Recordist",
        image: imageMap["15470"],
        bio: [
            "John Baptist Gichengo is a Kenyan sound recordist and camera assistant with over four years of experience working across documentary, broadcast, and branded content productions. Active in the industry since 2020, he has contributed to both local and international projects, supporting high-quality storytelling through precise sound capture and dependable camera support.",
            "He has worked on productions for respected organizations and platforms including National Geographic Kids (2022-2023), the Olympic Channel (Girls Rule), BBC-affiliated projects, AGRA, British Red Cross, and Standard Chartered. His documentary credits include 'Our Land, Our Freedom', directed by Zippy Kimundu and Mina Nanji, where he served as sound recordist.",
            "Baptist John is known for his strong on-location recording skills, attention to detail, and ability to work efficiently in demanding field environments. He is comfortable collaborating with directors, cinematographers, and producers to ensure clean audio and smooth production workflows, particularly in documentary and factual storytelling contexts.",
            "With a growing focus on impactful, socially grounded narratives, he continues to build his career in cinematography and sound, bringing professionalism, adaptability, and technical reliability to every project.",
            "Contact: johnbaptizo4@gmail.com | 0748 794 366"
        ]
    },
    {
        id: "new-1",
        name: "Cornelius Oburu",
        role: "Editor",
        image: "oburu.jpg",
        bio: ["Oburu is a highly imaginative filmmaker with a deep passion for color grading and a strong aptitude for documentary film editing. He has contributed to a range of notable projects in both disciplines, with his most recent and prominent works being 'Widow Champion' and 'The Christmas Run', MTV SHUGA series and the feature film ‘Shimoni’."]
    },
    {
        id: "new-2",
        name: "Ivy B Nakhadu",
        role: "Production Assistant",
        image: "ivy.jpg",
        bio: ["I’m a versatile production professional with experience across film, television, and live broadcast in East Africa and beyond. From audience handling on BBC Sema Kenya and 77 Percent – Deutsche Welle to behind-the-scenes logistics with Afro Films International and On Screen Productions, I’ve built a reputation for being calm, prepared, and deeply reliable on set. Whether managing crew schedules, wrangling live audiences, or coordinating real-time resources, I bring clear communication, grounded energy, and a genuine love for collaborative storytelling. With a background in both production and hospitality, I thrive where creativity meets chaos—and I always show up ready to deliver."]
    },
    {
        id: "new-3",
        name: "Hazel Kipasaina",
        role: "Social Media",
        image: "hazel.jpg",
        bio: ["I’m a social media storyteller with a passion for film and visual narratives. I love bringing untold stories to life, giving them a voice, and creating spaces where powerful stories can be seen, felt, and heard."]
    }
];
